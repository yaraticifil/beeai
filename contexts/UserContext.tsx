...
function applyBeeXPInternal(bees: BeeAgent[], role: BeeRole, amount: number): BeeAgent[] {
  return bees.map((b) => {
    if (b.role === role) {
...
export function UserProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (stored) {
        ...
        const hydrated: User = {
          ...
          honeyBoosterUntil: parsed.honeyBoosterUntil ?? (parsed.doubleNextHoney ? Date.now() + 30 * 60 * 1000 : null),
        };
  ...
  const addHoney = async (amount: number) => {
    ...
    const isBoosted = user.honeyBoosterUntil ? Date.now() < user.honeyBoosterUntil : false;
  ...
  const spin = async (): Promise<{ pointsWon: number; prize: string } | null> => {
    ...
    if ((selected as any).bonus === "flower") {
      ...
      const isBoosted = updated.honeyBoosterUntil ? Date.now() < updated.honeyBoosterUntil : false;
  ...
  const addCheck: UserContextValue["addCheck"] = async (input) => {
    ...
    const bees = applyBeeXPInternal(user.bees || [], "Kâtip", XP_REWARDS.CHECK_ADDED);
  ...
  const requestRevision: UserContextValue["requestRevision"] = async (checkId) => {
    ...
    const updatedBees = applyBeeXPInternal(user.bees || [], "Aracı", XP_REWARDS.REVISION);
  ...
  const pickOffer: UserContextValue["pickOffer"] = async (checkId, offerId, couponId) => {
    ...
    if (couponId) {
      const couponIdx = updatedCoupons.findIndex(c => c.id === couponId && !c.used);
      if (couponIdx >= 0) {
        const coupon = updatedCoupons[couponIdx];
        let netPay = offer.netPay;
        if (coupon.kind === 'discount') {
          // Bonus: Reduce the factoring cost (gap between amount and netPay)
          const cost = check.amount - offer.netPay;
          const bonus = Math.round(cost * (coupon.value / 100));
          netPay = offer.netPay + bonus;
        } else if (coupon.kind === 'fee') {
          // Fixed bonus on net payout
          netPay = offer.netPay + coupon.value;
        }

        offer = { ...offer, netPay, notes: `${offer.notes || ''} (${coupon.title} uygulandı)`.trim() };
        updatedCoupons = [...updatedCoupons];
        updatedCoupons[couponIdx] = { ...coupon, used: true };
      }
    }
    ...
    let updatedBees = applyBeeXPInternal(user.bees || [], "Aracı", XP_REWARDS.OFFER_PICKED);
    updatedBees = applyBeeXPInternal(updatedBees, "İzci", XP_REWARDS.OFFER_PICKED);
  ...
  const awardBeeXP: UserContextValue["awardBeeXP"] = async (role, xp) => {
    if (!user) return;
    const bees = applyBeeXPInternal(user.bees || [], role, xp);
