...
function calculateHoneyGain(user: User, baseAmount: number): number {
  const isBoosted = user.honeyBoosterUntil > Date.now();
  return isBoosted ? baseAmount * 2 : baseAmount;
}

function awardBeeXPInternal(user: User, roleOrRoles: BeeRole | BeeRole[], xp: number): { bees: BeeAgent[]; activities: User["activities"] } {
  const roles = Array.isArray(roleOrRoles) ? roleOrRoles : [roleOrRoles];
  const newActivities = [...(user.activities || [])];
    ...
    if (roles.includes(b.role)) {
      ...
      while (newXp >= 100) {
        ...
        newActivities.unshift({
          id: `act_lvl_${Date.now()}_${b.role}_${newLevel}`,
          type: "level_up",
          message: `Tebrikler! ${b.name} ${newLevel}. seviyeye ulaştı! 🥳`,
          time: Date.now(),
        });
      }
  ...
  return { bees: updatedBees, activities: newActivities.slice(0, 10) };
}
...
export function UserProvider({ children }: { children: ReactNode }) {
    ...
    try {
      ...
      if (stored) {
        ...
        const hydrated: User = {
          ...
          ...parsed,
          lastDailySpinGrantDate: parsed.lastDailySpinGrantDate || (parsed as any).lastSpinDate || "",
  ...
  const addHoney = async (amount: number) => {
    ...
    const gain = calculateHoneyGain(user, amount);
  ...
  const checkDailySpins = async () => {
    ...
    if (user.lastDailySpinGrantDate !== today) {
      await saveUser({ ...user, spinCount: user.spinCount + 3, lastDailySpinGrantDate: today });
    }
  };
  ...
  const spin = async (): Promise<{ pointsWon: number; prize: string } | null> => {
    ...
    let updated: User = { ...user, spinCount: newSpinCount, lastDailySpinGrantDate: today };
    ...
    if ((selected as any).bonus === "flower") {
      ...
      const gain = calculateHoneyGain(updated, pointsWon);
  ...
  const harvestFlower = async (flowerId: string): Promise<number> => {
    ...
    const honeyEarned = calculateHoneyGain(user, baseEarned);
  ...
  const harvestAllFlowers = async (): Promise<number> => {
    ...
    readyFlowers.forEach(() => {
      const baseEarned = Math.floor(Math.random() * 16) + 15;
      totalEarned += calculateHoneyGain(user, baseEarned);
    });
  ...
  const addCheck: UserContextValue["addCheck"] = async (input) => {
    ...
    const scribe = (user.bees || []).find(b => b.role === "Kâtip");
    const honeyBonus = scribe ? scribe.level * 5 : 0;

    const { bees: updatedBees, activities: updatedActivities } = awardBeeXPInternal(user, "Kâtip", XP_REWARDS.CHECK_ADDED);

    const updated: User = {
      ...user,
      honeyPoints: user.honeyPoints + honeyBonus,
      level: computeLevel(user.honeyPoints + honeyBonus),
      bees: updatedBees,
      ...
      activities: [
        {
          id: `act_${Date.now()}`,
          type: "check_add",
          message: `${check.issuerName} firmasına ait çek eklendi.${honeyBonus > 0 ? ` (+${honeyBonus} 🍯)` : ""}`,
          time: Date.now(),
        },
        ...updatedActivities,
      ].slice(0, 10),
  ...
  const ensureOfferProgress: UserContextValue["ensureOfferProgress"] = async (checkId) => {
    ...
    // Pilot: 🕵️‍♂️ İzci arı hızı (Seviye başına %10 hızlanma, max %50)
    const scout = (user.bees || []).find(b => b.role === "İzci");
    const speedMultiplier = scout ? Math.max(0.5, 1 - (scout.level - 1) * 0.1) : 1;

    // Pilot hızlandırılmış: teklifler 15s, 45s, 90s
    const t1 = 15000 * speedMultiplier;
    const t2 = 45000 * speedMultiplier;
    const t3 = 90000 * speedMultiplier;
    ...
    if (newOffers.length === 0 && status === req.status) return;
  ...
  const requestRevision: UserContextValue["requestRevision"] = async (checkId) => {
    ...
    const { bees: updatedBees, activities: updatedActivities } = awardBeeXPInternal(user, "Aracı", XP_REWARDS.REVISION);
  ...
  const pickOffer: UserContextValue["pickOffer"] = async (checkId, offerId, couponId) => {
    ...
    const { bees: updatedBees, activities: updatedActivities } = awardBeeXPInternal(user, ["Aracı", "İzci"], XP_REWARDS.OFFER_PICKED);
  ...
  const awardBeeXP: UserContextValue["awardBeeXP"] = async (role, xp) => {
    if (!user) return;
    const { bees, activities } = awardBeeXPInternal(user, role, xp);
    await saveUser({ ...user, bees, activities });
  };
  ...
  const checkPulseXP = async (): Promise<boolean> => {
    ...
    const { bees, activities } = awardBeeXPInternal(user, "Nabız", XP_REWARDS.PULSE_CHECKED);
    await saveUser({ ...user, bees, activities, lastPulseCheckDate: today });
    return true;
  };
