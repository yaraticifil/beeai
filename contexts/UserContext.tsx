...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const pickOffer: UserContextValue["pickOffer"] = async (checkId, offerId, couponId) => {
    ...
    const originalOffer = req.offers.find((o) => o.id === offerId);
    if (!originalOffer) return;

    const check = (user.checks || []).find((c) => c.id === checkId);
    if (!check) return;

    let offer = { ...originalOffer };
    let coupons = [...(user.coupons || [])];

    if (couponId) {
      const cIdx = coupons.findIndex((c) => c.id === couponId && !c.used);
      if (cIdx >= 0) {
        const coupon = coupons[cIdx];
        coupons[cIdx] = { ...coupon, used: true };

        if (coupon.kind === "discount") {
          const factoringCost = check.amount - offer.netPay;
          const discount = Math.round(factoringCost * (coupon.value / 100));
          offer.netPay += discount;
          offer.notes = (offer.notes ? offer.notes + " + " : "") + `%${coupon.value} Kupon İndirimi`;
        } else if (coupon.kind === "fee") {
          offer.netPay += coupon.value;
          offer.notes = (offer.notes ? offer.notes + " + " : "") + `₺${coupon.value} Bonus`;
        }
      }
    }
