...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const addCheck: UserContextValue["addCheck"] = async (input) => {
    ...
    const newPoints = user.honeyPoints + bonusHoney;
  ...
  const pickOffer: UserContextValue["pickOffer"] = async (checkId, offerId, couponId) => {
    ...
    if (couponId) {
      ...
      if (coupon) {
        // Apply coupon effect
        let newNetPay = offer.netPay;
        if (coupon.kind === "discount") {
          // Discount on factoring cost (Amount - NetPay)
          const cost = check.amount - offer.netPay;
          const discount = Math.round(cost * (coupon.value / 100));
          newNetPay = offer.netPay + discount;
        } else if (coupon.kind === "fee") {
          newNetPay = offer.netPay + coupon.value;
        }

        offer = { ...offer, netPay: newNetPay, notes: `${offer.notes || ""} (${coupon.title} kullanıldı)`.trim() };
