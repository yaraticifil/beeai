...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const addHoney = async (amount: number) => {
    ...
    const updated: User = {
      ...
      honeyBoosterUntil: isBoosted ? user.honeyBoosterUntil : 0,
    };
    await saveUser(updated);
  };
  ...
  const addCheck: UserContextValue["addCheck"] = async (input) => {
    ...
    let updated = applyBeeXPInternal(user, "Kâtip", XP_REWARDS.CHECK_ADDED);
    updated = {
      ...updated,
  ...
  const requestRevision: UserContextValue["requestRevision"] = async (checkId) => {
    ...
    let updated = applyBeeXPInternal(user, "Aracı", XP_REWARDS.REVISION);
  ...
  const pickOffer: UserContextValue["pickOffer"] = async (checkId, offerId) => {
    ...
    let updated = applyBeeXPInternal(user, "Aracı", XP_REWARDS.OFFER_PICKED);
  ...
  const applyBeeXPInternal = (currentUser: User, role: BeeRole, xp: number): User => {
    let leveledUp = false;
    let leveledBeeName = "";
    let newLevel = 1;

    const bees = (currentUser.bees || []).map((b) => {
      if (b.role === role) {
        let nXp = b.xp + xp;
        let nLvl = b.level;
        if (nXp >= 100) {
    ...
    if (leveledUp) {
      activities.unshift({
        id: `lvl_${Date.now()}_${role}`,
        type: "bee_level_up",
        message: `Tebrikler! ${leveledBeeName} ${newLevel}. seviyeye ulaştı!`,
        time: Date.now(),
      });
    }

    return { ...currentUser, bees, activities: activities.slice(0, 10) };
  };

  const awardBeeXP: UserContextValue["awardBeeXP"] = async (role, xp) => {
    if (!user) return;
    const updated = applyBeeXPInternal(user, role, xp);
    await saveUser(updated);
  };
