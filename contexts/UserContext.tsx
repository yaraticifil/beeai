...
function awardBeeXPInternal(user: User, roleOrRoles: BeeRole | BeeRole[], xp: number): User {
    ...
    if (roles.includes(b.role)) {
      ...
      if (newXp >= 100) {
        ...
        newActivities.unshift({
          id: `lvlup_${Date.now()}_${b.role}`,
          type: "level_up",
          message: `${b.name} seviye atladı! Yeni Seviye: ${newLevel}`,
          time: Date.now(),
        });
      }
...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const spin = async (): Promise<{ pointsWon: number; prize: string } | null> => {
    ...
    if ((selected as any).bonus === "flower") {
      ...
      updated = { ...updated, honeyBoosterUntil: Math.max(user.honeyBoosterUntil, Date.now()) + 15 * 60 * 1000 };
    } else if (pointsWon > 0) {
  ...
  const ensureOfferProgress: UserContextValue["ensureOfferProgress"] = async (checkId) => {
    ...
    // Scout bonus: Reduce wait times by 10% per level above 1, max 50% speedup.
    const scout = (user.bees || []).find(b => b.role === "İzci");
    const speedMultiplier = scout ? Math.max(0.5, 1 - (scout.level - 1) * 0.1) : 1;

    // Pilot hızlandırılmış: teklifler 15s, 45s, 90s (adjusted by scout speed)
    const targetCount = elapsed >= 90000 * speedMultiplier ? 3 : elapsed >= 45000 * speedMultiplier ? 2 : elapsed >= 15000 * speedMultiplier ? 1 : 0;

    if (targetCount === req.offers.length && req.status !== "collecting" && now <= req.deadlineAt) {
        // No new offers and status is consistent
        return;
    }
  ...
  const checkPulseXP = async (): Promise<boolean> => {
    ...
    const updated = awardBeeXPInternal(user, "Nabız", XP_REWARDS.PULSE_CHECKED);
    await saveUser({ ...updated, lastPulseCheckDate: today });
    return true;
  };
