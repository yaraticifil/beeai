    ...
    try {
      ...
      if (stored) {
        ...
        const hydrated: User = {
          ...
          honeyBoosterUntil: parsed.honeyBoosterUntil ?? ((parsed as any).doubleNextHoney ? Date.now() + 60000 : 0),
        };
    ...
    if (couponId) {
      ...
      if (cIdx >= 0) {
        ...
        if (coupon.kind === "discount") {
          const gap = check.amount - offer.netPay;
          const bonus = Math.round(gap * (coupon.value / 100));
          offer.netPay += bonus;
          offer.notes = (offer.notes ? offer.notes + " | " : "") + `%${coupon.value} indirim uygulandı`;
        } else if (coupon.kind === "fee") {
          offer.netPay += coupon.value;
          offer.notes = (offer.notes ? offer.notes + " | " : "") + `${coupon.value} TL masraf iadesi`;
        }
      }
    }
