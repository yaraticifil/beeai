...
function computeDailyPulseInternal(companyName?: string): DailyPulse {
  ...
  const isFestival = (h % 100) < 10;

  const note = isFestival
    ? "Hasat Bayramı! Bugün bahçeden toplanan tüm çiçekler 2 kat bal kazandırıyor. Bereketli olsun!"
    : mood === "sert"
...
export function UserProvider({ children }: { children: ReactNode }) {
  ...
  const checkDailyLogins = async () => {
    ...
    const activityMsg = `Günlük giriş yapıldı! Seriniz: ${newStreak} gün. ${bonusHoney} Bal kazandınız.`;
    updated = addActivityInternal(updated, "login", activityMsg);
  ...
  const harvestAllFlowers = async (): Promise<number> => {
    ...
    let updatedWithActivity = addActivityInternal({
      ...
      missions: updatedMissions,
    }, "harvest", pulse.isFestival ? "Hasat Bayramı bereketiyle çiçekler toplandı!" : (hasGolden ? "Çiçekler hasat edildi (Altın çiçekler dahil)!" : "Tüm çiçekler hasat edildi."));
  ...
  const ensureOfferProgress: UserContextValue["ensureOfferProgress"] = async (checkId) => {
    ...
    // Synergy: İzci & Aracı sv 3+ ise ek %5 hız
