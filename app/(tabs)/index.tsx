...
import { impactLight, impactMedium, notificationSuccess } from "@/shared/utils/haptics";
...
function StatCard({ label, value, icon, delay = 0, onPress }: { label: string; value: string; icon: keyof typeof Ionicons.glyphMap; delay?: number; onPress?: () => void }) {
  return (
    <Animated.View entering={FadeInDown.delay(delay).springify()} style={{ flex: 1 }}>
      <Pressable onPress={onPress} style={({ pressed }) => [{ flex: 1 }, pressed && onPress && { opacity: 0.8 }]}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.statGrid}>
            ...
            <View style={styles.statRow}>
               <StatCard label="Toplanıyor" value={String(activeReqs)} icon="time" delay={200} onPress={() => router.push("/(tabs)/offers")} />
               <StatCard label="Hazır" value={String(offersReady)} icon="flash" delay={250} onPress={() => router.push("/(tabs)/offers")} />
            </View>
          </View>
        </View>
